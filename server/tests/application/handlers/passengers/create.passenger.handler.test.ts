...
describe('Create Passenger', () => {
  it('should create a passenger', async () => {
    ...
    const result = await createPassengerHandler.execute(
      new CreatePassengerCommand('Pepito perez', 'pepitoperez@yopmail.com', '12345678'),
    );
  ...
  it('should throw an error if identity card is invalid', async () => {
    ...
    const result = await expect(createPassengerHandler.execute(command)).rejects.toStrictEqual(
